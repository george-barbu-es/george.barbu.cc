...
return (
  <section className={className}>
      <>
        <h2 className="uppercase font-norwester text-xl text-primary mb-4">{data[0].label}</h2>
          ...
          <div key={index}>
            {groups[item].length > 1 && ( 
              <div key={index}  className={" bi-avoid bb-always font-lato mt-11"}>
                ...
                <div className="relative border-l border-solid border-primary">
                  {groups[item].map((item, index) => (
                    <div key={index}  className={"bi-avoid bb-always font-lato ml-4 " + (index !== 0 ? 'mt-11' : 'mt-4')}>
                      <div className="absolute w-3 h-3 border-primary bg-primary rounded-full mt-1.5 -left-1.5 border"></div>
            ...
            {groups[item].length === 1 && (  
              <div key={index}  className={"bi-avoid bb-always font-lato " + (index !== 0 ? 'mt-11' : '')}>
                ...
                <div className="text-primary text-xs font-semibold mb-4">
                  <span className="border-r-2 border-solid border-primary mr-1 pr-1">{groups[item][0].company}</span>       
                  <span className="">
